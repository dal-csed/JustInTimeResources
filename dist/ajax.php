...
if($act == 'like'): //if the user click on "like"
	if(($like_count == 0) && ($dislike_count == 0)){
		mysqli_query($link,'INSERT INTO rating (id_item, ip, rate )VALUES("'.$ID.'", "'.$user_ip.'", "1")');
	}
	if($dislike_count == 1){
		mysqli_query($link,'UPDATE rating SET rate = 1 WHERE id_item = '.$ID.' and ip ="'.$user_ip.'"');
	}
endif;

if($act == 'dislike'): //if the user click on "like"
	if(($like_count == 0) && ($dislike_count == 0)){
		mysqli_query($link,'INSERT INTO rating (id_item, ip, rate )VALUES("'.$ID.'", "'.$user_ip.'", "2")');
	}
	if($like_count == 1){
		mysqli_query($link,'UPDATE rating SET rate = 2 WHERE id_item = '.$ID.' and ip ="'.$user_ip.'"');
	}
endif;
?>
